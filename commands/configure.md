...
1. **Ask the user to pick a preset** with `AskUserQuestion`, single-select:
   ...
   - `full` — all 176 tools (current default if the user has never configured this)
...
- Keep the conversation tight. Do not dump the full 176-tool list — summarise the chosen preset and let the user opt into fine-tuning.
