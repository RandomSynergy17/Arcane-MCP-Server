...
MCP server for Docker management via the Arcane platform. 174 tools, 2 resources, 4 prompts, companion skill, Claude Code plugin.
...
### Tools (174 across 25 modules in `src/tools/`)
...
6. Verify: `npm run build && grep -rc 'register("arcane_' dist/tools/*.js | awk -F: '{sum+=$2} END {print sum}'` (expect 174)
